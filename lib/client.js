window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const registerMentionEvents = (events) => {
        if (!events || typeof events.register !== "function") return;
        events.register({
          kind: "mentionedPaths",
          match: (event) => {
      ...
      const convEvents = ctx.get("conversationEvents") || (ctx.uiConversation && ctx.uiConversation.events) || (typeof ctx.get === "function" && ctx.get("uiConversation") && ctx.get("uiConversation").events);
      if (convEvents !== undefined) {
        registerMentionEvents(convEvents);
      } else if (typeof ctx.inject === "function") {
        ctx.inject(["uiConversation"], (convCtx) => {
          if (convCtx.uiConversation && convCtx.uiConversation.events) {
            registerMentionEvents(convCtx.uiConversation.events);
          }
        });
      }

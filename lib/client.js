window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // ── turnTail：本轮提到且真实存在的文件 ──
      // 0.1.7 回归修复（2026-09-24 实测）：官方把 conversation.chat.turnTail 从
      // **chain 槽**改成 **list 槽**（`kind: "list"`，见 dsh-client-ui-chat 的 turn-tail 节点）。
      // chain 要 `select`、list 必须给 `options.id` —— 旧写法（只给 select、无 id）在 0.1.7 上
      // 每渲染一次就抛 `list slot "conversation.chat.turnTail" requires options.id`，功能整体失效。
      // list 语义下没有 select 回调 → 把取数逻辑搬进组件；用 useMemo + 内容键保住引用稳定，
      // 否则每次渲染都会重新 POST 一次 /check。没有可显示内容时返回 null（list 槽允许空渲染）。
      ...
      slots.inject("conversation.chat.turnTail", () => {
        ...
        return slots.register(
        {
          ...
          id: "dsh-file-mentions-turn-tail",
        },
        (props) => {
          ...
          // 本轮提到的路径（原 select 回调的逻辑；内容键保证引用稳定）
          const mentioned = props.turn && props.turn.data ? props.turn.data.get("mentionedPaths") : undefined;
          const rawPaths = mentioned && Array.isArray(mentioned.paths) ? mentioned.paths : [];
          const pathsKey = rawPaths.join("\u0000");
          const matched = react.useMemo(() => rawPaths, [pathsKey]);
      ...
      const ROW_CONFIG_KEY = "dsh-file-mentions#file-mentions";
      slots.inject("plugins.row.config", () => slots.register(
        { name: "plugins.row.config", key: ROW_CONFIG_KEY, order: 30, label: "文件提及" },
      ...
      slots.inject("plugins.bundle.config", () => slots.register(
        { name: "plugins.bundle.config", key: "dsh-file-mentions", order: 30, label: "文件提及" },
        () => react.createElement(SettingsCard, { inCard: true }),
      ));

    ...
    function apply(ctx) {
      ...
      function verifyPaths(pending) {
        ...
        Promise.all(ids.map((id) =>
          ...
          schedulePathCodes(); // 重跑：缓存已就绪，打标记
        });
      }
      // ── rAF 合帧调度（v1.0.12，方案 B）：一帧最多跑一次 processPathCodes ──
      // observer 回调不再同步全文档扫描；与 renderBareHighlights 同机制，
      // 高频 mutation（流式输出/滚动/动画）下从"每变动 O(文档)"降为"每帧 O(消息区)"。
      let pathScanQueued = false;
      function schedulePathCodes() {
        if (pathScanQueued) return;
        pathScanQueued = true;
        requestAnimationFrame(() => {
          pathScanQueued = false;
          processPathCodes();
        });
      }
      ...
      function processPathCodes() {
        ...
        const nodes = scope.querySelectorAll("code");
        const pending = [];
      ...
      function collectBareHits() {
        // 装饰边界 = 消息区（[data-conversation-scroll] 容器内）；消息区内的
        // code/pre/a/button 等仍跳过（code 路径走 processPathCodes，避免重复装饰；
        // 官方产物按钮/链接不碰）。消息区外（侧栏/hover 卡/菜单/设置页）零扫描。
        const scope = messageScope();
        if (scope === null) return [];
      ...
      ctx.effect(() => {
        ...
        // 只用 childList 覆盖结构变化（消息追加/切换会话等）。回调经 rAF 合帧：
        // processPathCodes 与 renderBareHighlights 各一帧至多一次（v1.0.12）。

    ...
    function apply(ctx) {
      ...
      function onDocumentClick(event) {
        ...
        if (inline !== null) {
          ...
          if (raw && currentSessionId !== null) {
            fetch("/api/file-mentions/open", {
              method: "POST",
              headers: { "content-type": "application/json" },
              body: JSON.stringify({ sessionId: currentSessionId, path: raw, mode: "open" }),
              cache: "no-store",
            }).catch(() => {});
          }
          return;
        ...
        // 直接请求 open 路由；路径不存在时 host 返回 404，静默无副作用
        fetch("/api/file-mentions/open", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ sessionId: currentSessionId, path: text, mode: "open" }),
          cache: "no-store",
        }).catch(() => {});
      }
      ctx.effect(() => {
        ...
        for (const code of nodes) {
          ...
          btn.addEventListener("click", (event) => {
            ...
            fetch("/api/file-mentions/open", {
              method: "POST",
              headers: { "content-type": "application/json" },
              body: JSON.stringify({ sessionId: currentSessionId, path: text, mode: "reveal" }),
              cache: "no-store",
            }).catch(() => {});
          });
      ...
      slots.inject("conversation.chat.turnTail", () => slots.register(
        ...
        (props) => {
          ...
          const systemOpen = (p, mode) => {
            fetch("/api/file-mentions/open", {
              method: "POST",
              headers: { "content-type": "application/json" },
              body: JSON.stringify({ sessionId, path: p, mode: mode || "open" }),
              cache: "no-store",
            }).catch(() => {});
          };

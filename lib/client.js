window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // React 管理的输入区绝不能由本插件改写 DOM：textarea/input/contenteditable 以及

    // 输入框的 mirror/backdrop 都是 React 子树的可见宿主节点，插入 span 会导致

    // React 重渲染时 `removeChild` NotFoundError（输入 `/plan` 后按空格即触发）。

    function shouldSkipPathNode(parentEl) {

      if (parentEl === null || parentEl === undefined) return true;

      return parentEl.closest(

        "textarea, input, [contenteditable], [data-input-mirror], [data-input-backdrop], [data-composer-card], [class$='_input'], [data-conversation-scroll]"
      ...
      function processBarePaths() {
        ...
        const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
          ...
          acceptNode(node) {
            ...
            if (shouldSkipPathNode(p)) return NodeFilter.FILTER_REJECT;
